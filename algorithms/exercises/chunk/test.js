...
test('chunk divides an array of 5 elements with chunk size 1', () => {
    const arr = [1, 2, 3, 4, 5];
    const chunked = chunk(arr, 1);
...
test('chunk handles bad input', () => {
    chunk([], 1);
    chunk([1, 2, 3], -1);
    chunk(null, -1);

    expect(console.log.mock.calls[0][0]).toEqual('Bad input');
    expect(console.log.mock.calls[1][0]).toEqual('Bad input');
    expect(console.log.mock.calls[2][0]).toEqual('Bad input');
});
